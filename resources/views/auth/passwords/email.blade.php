            ...
            <div class="d-flex col-lg-4 align-items-center auth-bg px-2 p-lg-2">
              <div class="col-12 col-sm-8 col-md-6 col-lg-12 px-xl-2 mx-auto">
                <div class="shadow p-2 mb-1 bg-white rounded">
                    ...
                    <h2 class="card-title fw-bold mb-1">{{ __('Forgot your Password?') }}</h2>
                    <p class="card-text mb-2">{{ __("Please enter your email address. You will receive a link to reset your password.")}}</p>
                    ...
                    @if (session('status'))
                        ...
                        @endif
                    <form class="auth-forgot-password-form mt-2" action="{{ route('recupera.password') }}" method="POST">

                        @csrf
                      <div class="mb-1">
                        <label class="form-label" for="password">{{ __('Email Address') }}</label>
                        <input class="form-control @error('email') is-invalid @enderror" id="email" type="text" name="email" placeholder="" aria-describedby="email" autofocus="" tabindex="1" value="{{ old('email') }}"/>
                        ...
                        @endif
                      <div align="right">
                        <button class="btn btn-primary w-50" tabindex="2"> {{ __('Send') }}</button>
                      </div>
